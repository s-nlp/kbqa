...
class QuestionToRankInstanceOf(_QuestionToRankBase):
    ...
    def _split_toks(
            ...
            return [
                ...
                if QuestionToRankInstanceOf.stemmer.stem(tok.lower()) not in QuestionToRankInstanceOf.stopwords
            ]
    ...
    def _select_answer_instance_of(
        ...
        sentence_embeddings = QuestionToRankInstanceOf._calculate_sent_scores(tuple([str(e.label) for e, _ in answer_instance_of_count]))
        scores_collection = cosine_similarity(
            sentence_embeddings[:initial_number],
            sentence_embeddings[initial_number:]
        )

        selected_entities = [e for e, _ in answer_instance_of_count[:initial_number]]
        for scores in scores_collection:
            for e, _ in np.array(answer_instance_of_count[initial_number:])[scores > th]:
    ...
    def _calculate_property_question_intersection_score(property, question):
        # property_toks = QuestionToRankInstanceOf._split_toks(str(property.label))
        # question_toks = QuestionToRankInstanceOf._split_toks(str(question))
        # property_question_intersection_score = len(set(property_toks).intersection(question_toks))
        # return property_question_intersection_score
        sentence_embeddings = QuestionToRankInstanceOf.sent_transformer.encode([question, property.label])
        ...
        if self._final_answers is None:
            ...
            for q_entity in self.question_entities:
                if self.only_forward_one_hop:
                    ...
                    neighbors = q_entity.forward_one_hop_neighbors + q_entity.backward_one_hop_neighbors
                ...
                for property, entity in neighbors:
                    ...
                    instance_of_score = len(set(entity.instance_of).intersection(self.answer_instance_of))
                    if instance_of_score > 0:
                        instance_of_score = (len(self.answer_instance_of) - instance_of_score) / len(self.answer_instance_of)
                    # ---
                    ...
                    if entity in self.answers_candidates:
                        ...
                        answers_candidates_score = (count - self.answers_candidates.index(entity)) / count
                        answers_candidates_score = answers_candidates_score
                    else:
            ...
            for entity in self.answers_candidates:
                ...
                selected_set.append((None, entity, instance_of_score, 0, answers_candidates_score, 0))
