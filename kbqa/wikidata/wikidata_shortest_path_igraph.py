...
from igraph import Graph  # pylint: disable=import-error
import time
import igraph  # pylint: disable=import-error
...
class WikidataShortestPathIgraphCache(WikidataBase):
    ...
    def __init__(
        ...
        print("loading igraph")
        self.subgraph = Graph.Read_Ncol(
            graph_path, names=True, directed=True, weights=True
        )
        print(f"took {time.time() - start_time} to load igraph.")
        self.cache = {}
    ...
    def get_edge(self, entity1: str, entity2: str):
        ...
        try:
            ...
            print(f"no edge between {entity1}->{entity2}")
            return None
