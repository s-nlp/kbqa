...
class NerToSentenceInsertion(CacheBase):
    ...
    """Module for adding START and END tokens for entities in the initial sentence,
    after that all first letters are capitalized"""
    ...
    def entity_labeling(self, test_question):
        ...
        if test_question not in self.cache:
            ...
            if entities != '':
                ...
                ner_question = test_question[:index] + \
                               '[START] ' + test_question[index:index + len(entities)] + \
                               ' [END]' + test_question[index + len(entities):]
            else:
